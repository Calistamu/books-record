...
包、帧、数据报、段、消息：  
&ensp;&ensp;帧用于表示数据链路层中包的单位。数据报是IP和UDP等网络层以上的分层中包的单位。段表示TCP数据流中的信息。消息是指应用协议中数据的单位。  

## 第3章 数据链路

mac地址48比特，其中3~24位表示厂商识别码（官方叫OUI），25~48表示厂商内部为识别每个网卡而用。采用分层管理。
*  实际上mac并不总是唯一，只要不属于同一个数据链路就不会出现问题。可以在微机板上自由设置自己的mac地址，一台主机上如果启动多个虚拟机，由于没有硬件的网卡只能由虚拟软件自己设定mac地址给多个虚拟网卡，因此很难保证mac地址唯一。
* 网络分析器，可以分析出局域网中的包是由哪个厂商的网卡发出的，它通过读取数据帧当中发送mac地址里的厂商识别码进行识别。
![](images/mac-address.jpg)

网络分为共享介质型（半双工通信）和非共享介质型（全双工通信）。共享介质型网络介质访问控制方式：争用方式（也叫CSMA载波监听多路访问，先到先得。CSMA/CD要求每个站提前检查冲突，一旦发生冲突，则尽早释放信道。）和令牌传递方式。

以太网交换机：多个端口的网桥。它们根据数据链路层中每个帧的目标mac地址，决定从哪个网络接口发生数据。这时所参考的、用以记录发送接口的表就叫做转发表。
* 交换机转发方式：存储转发和直通转发。  
  存储转发：检查以太网数据帧末尾的FCS位后再进行转发，可以避免发送由于冲突而被破坏的帧或噪声导致的错误帧。  
  直通转发：不需要将整个帧全部接收下来以后再进行转发，只需要得知目标地址即可开始转发。具有延迟较短的优势，但同时也不可避免地有发送错误帧的可能性。

解决网络中的环路问题：生成树和源路由（原理：搭建合适的环路，分散网络流量，在发生某一处路由故障时选择绕行，可以提高容灾能力。）

其他数据链路层协议  
1. ATM：以信元（ATM的一个信元只能发送固定的48字节数据）为单位进行传输的数据链路，由于线路占用时间短和能够高效传输大容量数据等特点主要用于广域网络的连接。
* ATM最大弊端：192个信元中只要有一个丢失，那么就要重发所有。
* 收敛状态：网络非常拥堵时，路由器或交换机无法完成包的处理，从而丢弃这些包。
2. POS：在SDH（在光纤上传输数字信号的物理层规范，即利用电话线或专线等可靠性较高的方式进行光传输的网络）上进行包通信的一种协议
3. FDDI分布式光纤数据接口：采用令牌环的访问方式。Token Ring的扩展产物。
4. Token Ring:现在人们已经不再采用令牌环技术。
5. 100VG-AnyLAN:采用扩展了令牌传递方式的需求优先访问方式。  
6. 光纤通道：实现高速数据通信的一种数据链路。
7. HIPPI：用于连接超大型计算机。
8. IEEE1394：面向家庭的局域网。
9. HDMI：高清晰度多媒体接口，通过一根缆线实现图像和声音等数字信号的高品质传输。
10. iSCSI:把SCSI的命令和数据包含进IP包内，进行数据传输。
11. InfiniBand:针对高端服务器的一种超高速传输接口技术。特点：高速、高可靠性以及低延迟，支持多并发链接。
12. DOCSIS：有线电视传输数据的行业标准。
13. 高速PLC：在家里或办公室内实现高速传输的一种通信方式。

## 第4章 IP协议

网络标识必须保证相互连接的每个段的地址不相重复，IP地址的主机标识不允许同一个网段内重复出现。
* IP地址四个级别
* 广播地址：将IP地址中的主机地址部分全部设置为1。广播地址分为本地广播和直接广播，在本网络内的广播叫本地广播，在不同网络之间的广播叫直接广播。
* 默认路由0.0.0.0/0或default。0.0.0.0的ip地址为0.0.0.0/32。

## 第5章 IP协议相关技术

RARP：ARP反过来，从mac地址定位ip地址的一种协议。

ICMP：辅助ip，分为通知出错原因的错误信息和用于诊断的查询消息。

DHCP服务器即使不在同一个链路上也可以实现统一分配和管理IP地址。

NAT：用于在本地网络中使用私有地址，在连接互联网时转而使用全局IP地址的技术。NAPT：转换TCP\UDP端口号。
...
* 毒性逆转和触发更新解决环路多余，需要长时间产生正确的路由信息的问题。

* RIP是选择路由器个数最少的路径，OSPF选择总的代价较小的路径。
OSPF：链路状态型路由协议。由于采用链路状态类型，即使网络中有环路，也能够进行稳定的路由控制，OSPF支持子网掩码。

BGP：边界网关协议，连接不同组织机构（或不同自治系统）的一种协议

MPLS：多协议标记交换技术，对每个IP包都设定一个叫做“标记”的值，再根据标记进行转发。MPLS优点：转发速度快，利用标记生成虚拟的路径，并在它上面实现IP等数据包的通信，因此“尽力而为”的IP网也可以提供基于MPLS的通信质量控制，带宽保证和VPN等功能。
